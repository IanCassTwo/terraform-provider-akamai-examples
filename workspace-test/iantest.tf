...
resource "akamai_property" "test-wheep-co-uk" {
  ...
  hostnames    = {
	"tf.wheep.co.uk" = "${akamai_edge_hostname.test-wheep-co-uk.edge_hostname}",
	"test.wheep.co.uk" = "${akamai_edge_hostname.test-wheep-co-uk.edge_hostname}"
  }
...
resource "akamai_property_activation" "test-wheep-co-uk" {
        ...
        contact = ["icass@akamai.com"]
        network = "${upper(terraform.workspace)}"
